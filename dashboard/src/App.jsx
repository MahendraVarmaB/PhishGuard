...
import { Shield, ShieldAlert, ShieldCheck, Activity, Download, Globe, Clock, Zap, AlertTriangle, Eye, Fingerprint } from 'lucide-react';
...
function App() {
  ...
  const avgLatency = history.length ? (history.reduce((a, b) => a + b.latency_ms, 0) / history.length).toFixed(0) : 0;
  ...
  // CTI source breakdown
  const ctiSources = {};
  history.forEach(h => {
    (h.threat_intel_matches || []).forEach(source => {
      if (source !== 'User Whitelist') {
        ctiSources[source] = (ctiSources[source] || 0) + 1;
      }
    });
  });
  const ctiBarData = Object.entries(ctiSources).map(([name, count]) => ({ name: name.length > 18 ? name.slice(0, 18) + '...' : name, count }));

  const exportIoC = () => {
    const malicious = history.filter(h => h.is_malicious);
    const iocs = malicious.map(h => ({
      indicator: h.url,
      type: "url",
      risk_score: h.risk_score,
      threat_intel: h.threat_intel_matches,
      typosquatting_target: h.typosquatting_target || null,
      domain_age_days: h.domain_age_days || null,
      is_obfuscated: h.is_obfuscated || false,
    ...
    a.href = url;
    a.download = `phishguard-ioc-report-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-[#060918] text-slate-200 font-sans">
      ...
      <div className="fixed inset-0 opacity-[0.03]" style={{ backgroundImage: 'linear-gradient(rgba(255,255,255,.1) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.1) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      ...
      <div className="relative z-10 p-6 lg:p-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={exportIoC} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-sm font-medium text-slate-300 transition-all duration-200">
              <Download size={16} />
              Export IoC
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-3 bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] rounded-2xl p-6">
            ...
            <div className="h-56 relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={65} outerRadius={85} paddingAngle={3} dataKey="value" strokeWidth={0}>
                    {pieData.map((entry, i) => (
            ...
            <div className="flex justify-center gap-6 mt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400">Safe ({benignScans})</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-red-500" />
                <span className="text-xs text-slate-400">Blocked ({maliciousScans})</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <MiniCard
              ...
              title="Typosquatting"
              desc="Brand impersonation attempts"
              value={typosquattingCount}
              color="amber"
              items={history.filter(h => h.typosquatting_target).slice(0, 3).map(h => `${h.typosquatting_target} ← ${new URL(h.url).hostname}`)}
            />
            <MiniCard
              icon={<Eye size={18} />}
              title="Obfuscated URLs"
              desc="Encoded or disguised links"
              value={obfuscatedCount}
              color="orange"
              items={history.filter(h => h.is_obfuscated).slice(0, 3).map(h => new URL(h.url).hostname)}
            />
            <MiniCard
              icon={<Clock size={18} />}
              title="New Domains"
              desc="Registered < 30 days ago"
              value={history.filter(h => h.domain_age_days !== null && h.domain_age_days < 30).length}
              color="rose"
              items={history.filter(h => h.domain_age_days !== null && h.domain_age_days < 30).slice(0, 3).map(h => `${new URL(h.url).hostname} (${h.domain_age_days}d)`)}
            />
            <MiniCard
              icon={<Globe size={18} />}
              title="CTI Hits"
              desc="URLhaus & VirusTotal matches"
              value={history.filter(h => h.threat_intel_matches?.some(m => m === 'URLhaus' || m === 'VirusTotal')).length}
              color="blue"
              items={history.filter(h => h.threat_intel_matches?.some(m => m === 'URLhaus' || m === 'VirusTotal')).slice(0, 3).map(h => new URL(h.url).hostname)}
            />
          </div>
        </div>
        ...
        <div className="mt-6 bg-white/[0.03] backdrop-blur-sm border border-white/[0.06] rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-white/[0.06] flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-slate-600 mt-0.5">Real-time URL analysis results</p>
            </div>
            <span className="text-xs text-slate-500">{totalScans} total entries</span>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-white/[0.02] sticky top-0 z-10">
                  ...
                  <th className="px-6 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">URL</th>
                  <th className="px-6 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Risk</th>
                  <th className="px-6 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                  <th className="px-6 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Intel</th>
                  <th className="px-6 py-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Latency</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.03]">
                {history.map((scan, i) => (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors cursor-pointer group" onClick={() => setSelectedScan(selectedScan === i ? null : i)}>
                    ...
                    <td className="px-6 py-3.5">
                      {scan.is_malicious ?
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold bg-red-500/10 text-red-400 rounded-md border border-red-500/20">
                          <div className="w-1.5 h-1.5 rounded-full bg-red-400" />BLOCKED
                        </span> :
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold bg-emerald-500/10 text-emerald-400 rounded-md border border-emerald-500/20">
                          <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />SAFE
                        </span>
                      }
                    </td>
                    <td className="px-6 py-3.5">
                      <div className="flex flex-wrap gap-1">
                        {scan.threat_intel_matches?.length > 0 ?
                          scan.threat_intel_matches.filter(m => m !== 'User Whitelist').map((m, j) => (
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-slate-600 text-xs">Browse the web with the PhishGuard extension to generate traffic.</p>
                      </div>
                    </td>
                  </tr>
                )}
        ...
        <div className="mt-8 text-center text-xs text-slate-700">
          PhishGuard v2.0 — ML-Powered Phishing Detection with Cyber Threat Intelligence
        </div>
      </div>
    </div>
  );
